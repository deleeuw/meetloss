...
. NLPCA maximizes the sum of the two largest eigenvalues of the correlation matrix of the variables. Before transformation the eigenvalues are `r val0`, after transformation they are `r val1`. The sum of the first two goes from `r val0[1]+val0[2]` to `r val1[1]+val1[2]`.
<hr>
...
In `r h$ntel` iterations we find minimum loss `r h$f`. The object scores are in figure
 and the transformation plots in figure
. The eigenvalues are now `r val2`, with sum of the first two equal to `r val2[1]+val2[2]`.
<hr>
