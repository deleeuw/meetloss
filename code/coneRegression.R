...
coneRegression <-
  function(theData,
           target,
           basis = matrix(theData, length(theData), 1),
           type = "i",
           ties = "s",
           missing = "m",
           itmax = 1000,
...
dykstra <- function(target, basis, theData, ties, itmax, eps) {
  x0 <- target
  itel <- 1
  ...
  repeat {
    ...
    a <- a + x1 - x0
    x2 <- isotone(theData, x1 - b, ties = ties)
    b <- b + x2 - x1
