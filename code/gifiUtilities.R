...
makeMissing <- function(theData, basis, missing) {
  there <- which(!is.na(theData))
  notthere <- which(is.na(theData))
...
interactiveCoding <- function(theData) {
  cmin <- apply(theData, 2, min)
  cmax <- apply(theData, 2, max)
  if (!all(cmin == 1))
    stop("data must be start at 1")
  nobs <- nrow(theData)
  h <- numeric(0)
  for (i in 1:nobs)
    h <- c(h, decode(theData[i, ], cmax))
  return(h)
}
...
profileFrequencies <- function(theData) {
  h <- interactiveCoding(theData)
  cmax <- apply(theData, 2, max)
  u <- unique(h)
  m <- length(u)
  g <- ifelse(outer(h, u, "=="), 1, 0)
  n <- colSums(g)
  h <- matrix(0, m, ncol(theData))
  for (j in 1:m)
    h[j, ] <- encode(u[j], cmax)
  return(list(h = h, n = n))
}
